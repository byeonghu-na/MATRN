...
class IterationCallback(LearnerTensorboardWriter):
    ...
    def _validate_test(self, dl=None, callbacks=None, metrics=None, keeped_items=False):
        "Validate on `dl` with potential `callbacks` and `metrics`."
        dl = ifnone(dl, self.learn.data.test_dl)
        metrics = ifnone(metrics, self.learn.metrics)
        cb_handler = CallbackHandler(ifnone(callbacks, []), metrics)
        cb_handler.on_train_begin(1, None, metrics); cb_handler.on_epoch_begin()
        if keeped_items: cb_handler.state_dict.update(dict(keeped_items=[]))
...
class DumpPrediction(LearnerCallback):
    ...
    def __init__(self, learn, dataset, charset_path, model_eval, image_only=False, debug=False, test_bh=None):
        ...
        if test_bh is None:
            ...
            self.dataset, self.root = dataset, Path(self.learn.path)/f'{dataset}-{self.model_eval}-{test_bh}'
